...
public class DefaultSelectionEventManager<T> implements CellPreviewEvent.Handler<T> {
  ...
  protected void handleMultiSelectionEvent(
    ...
    if (BrowserEvents.CLICK.equals(type)) {
      MouseEvent mouseEvent = (MouseEvent) nativeEvent;
      /*
       ...
       */
      boolean shift = mouseEvent.shiftKey;
      boolean ctrlOrMeta = mouseEvent.ctrlKey || mouseEvent.metaKey;
      boolean clearOthers =
      ...
      int keyCode = DOM.getKeyCode(nativeEvent);
      if (keyCode == 32) {
        /*
         ...
         */
        boolean shift = ((KeyboardEvent) nativeEvent).shiftKey;
        boolean clearOthers =
