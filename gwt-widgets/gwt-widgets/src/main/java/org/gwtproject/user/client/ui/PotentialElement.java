 ...
 *
 * <p>A simple {@link HTMLElement} implementation (<strong>not</strong> an actual dom object) that can
 ...
 * a real {@link HTMLElement} in place at the last moment.
 *
 * <p>TODO(rdcastro): Cover all unsupported methods with helpful error messages.
 */
public class PotentialElement extends HTMLElement {

  static {
    declareShim();
  }

  public static PotentialElement as(HTMLElement e) {
  ...
  /**
   * Tests whether a given {@link Object} represents a PotentialElement.
   *
   * @param o the {@link Object} to be tested
   ...
   */
  public static HTMLElement resolve(HTMLElement maybePotential) {
    return Js.<PotentialElement>uncheckedCast(maybePotential).resolve();
  }
  ...
  final HTMLElement setResolver(UIObject resolver) {
    Js.asPropertyMap(this).set("__gwt_resolve", buildResolveCallback(resolver));
    return this;
  }
